...
def generate_anchors(
    ...
    return _generate_anchors(
        stride,
        np.array(sizes, dtype=np.float64) / stride,  # Changed from np.float to np.float64
        np.array(aspect_ratios, dtype=np.float64),   # Changed from np.float to np.float64
    )
...
def _generate_anchors(base_size, scales, aspect_ratios):
    ...
    anchor = np.array([1, 1, base_size, base_size], dtype=np.float64) - 0.5  # Changed from np.float to np.float64
